...
Learn from https://github.com/golang-standards/project-layout/blob/master/README_zh-CN.md

## Module and CLI
go.mod has five type of settings
* go: Goland version
* module: A module directive defines the main module's path. A go.mod file must contain exactly one module directive.
* require: module dependency
* replace: A replace directive replaces the contents of a specific version of a module, or all versions of a module, with contents found elsewhere. The replacement may be specified with either another module path and version, or a platform-specific file path.
* exclude: An exclude directive prevents a module version from being loaded by the go command.
...
go mod tidy
# copy modules to project vendor folder and check
go mod vendor
# check if the modules has been downloaded, or downloaded modules has been modified
go mod verify
# view all modules
go list -m all
# view module graph
